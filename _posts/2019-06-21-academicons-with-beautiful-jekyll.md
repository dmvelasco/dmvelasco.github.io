...
After some searching and a few failed attempts, I hit upon how to make it work, and below have shared the steps taken to utilize Academicons with beautiful-jekyll with no need to hardcode each one into the theme.
...
Modify the file `_includes/footer.html` so that the type element from `_data/SocialNetworks.yml` is coded properly for the element icon.
...
Modify the file `_layouts/base.html` after the `common-ext-css:` line to	add the line `- "/css/academicons.css"` before `- "//maxcdn.bootstrapcdn.com/font-awesome/4.6.0/css/font-awesome.min.css"`. Both lines should have the same level of indentation.
